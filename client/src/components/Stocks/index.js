...
app.post("/stock", cors(), async (req, res) => {
  ...
  const request = await fetch(
    `https://www.alphavantage.co/query?function=${timePeriod(
      type
    )}&symbol=${ticker}&apikey=${process.env.REACT_APP_ALPHA_VANTAGE_API_KEY}`
  );
...
app.post("/stocks", async (req, res) => { //less than 5 stocks per minute
  ...
  let stocks = await tickers.map(async ticker => {
    const request = await fetch(
      `https://www.alphavantage.co/query?function=${timePeriod(type)}&symbol=${ticker}&apikey=${process.env.REACT_APP_ALPHA_VANTAGE_API_KEY}`
    );
    const data = await request.json();
    return data;
  });
